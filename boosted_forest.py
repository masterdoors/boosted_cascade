    ...
    def _fit_stage(
        ...
        for k in range(K):  
            ...
            for eid  in range(self.n_estimators):
                ...
                kfold_estimator.fit(X_aug.reshape(-1,X_aug.shape[2]), residual[:,:, k].reshape(-1,1), y, raw_predictions, rp_old.reshape(-1,residual.shape[2]), k, sample_weight)
                #kfold_estimator.update_terminal_regions(X_aug, y, raw_predictions, k)
                self.estimators_[i, k].append(kfold_estimator)
